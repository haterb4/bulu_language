...
impl Interpreter {
    ...
    pub fn new() -> Self {
        Self {
            ...
            promise_registry: std::sync::Arc::new(std::sync::Mutex::new(crate::runtime::promises::PromiseRegistry::new())),
    ...
    pub fn evaluate_call(&mut self, call_expr: &crate::ast::CallExpr) -> Result<RuntimeValue> {
        ...
        // For other calls, return a channel for now (stub)
        self.make_channel(crate::types::primitive::TypeId::Int32, None)
    }
    ...
    fn evaluate_make_call(&mut self, call_expr: &crate::ast::CallExpr) -> Result<RuntimeValue> {
        if call_expr.args.is_empty() {
            return Err(BuluError::Other("make() requires at least 1 argument".to_string()));
        }
        ...
        match &call_expr.args[0] {
            ...
            crate::ast::Expression::Identifier(ident) => {
                match ident.name.as_str() {
                    ...
                    "slice" => {
                        // make(slice, len, ...)
                        if args.len() < 2 {
                            return Err(BuluError::Other("make(slice) requires length argument".to_string()));
                        }
            ...
            crate::ast::Expression::Array(_) => {
                ...
                if args.len() < 2 {
                    return Err(BuluError::Other("make([size]T) requires size argument".to_string()));
                }
    ...
    fn extract_size_from_runtime_value(&self, value: &RuntimeValue) -> Result<usize> {
        match value {
            ...
            _ => Err(BuluError::Other("Size argument must be a non-negative integer".to_string())),
        }
    }

    /// Helper to parse type from expression
    fn parse_type_from_expression(&self, expr: &crate::ast::Expression) -> Result<crate::types::primitive::TypeId> {
        match expr {
            crate::ast::Expression::Identifier(ident) => {
                match ident.name.as_str() {
    ...
    fn create_channel(&self, element_type: crate::types::primitive::TypeId, capacity: Option<usize>) -> Result<RuntimeValue> {
    ...
    fn evaluate_promise_all(&mut self, call_expr: &crate::ast::CallExpr) -> Result<RuntimeValue> {
        if call_expr.args.len() != 1 {
            return Err(BuluError::Other("Promise.all() requires exactly one argument".to_string()));
        }
        ...
        if let crate::ast::Expression::Array(array) = array_expr {
            ...
            for element in &array.elements {
                ...
                match value {
                    ...
                    _ => return Err(BuluError::Other("Promise.all() requires an array of promises".to_string())),
                }
            }
            ...
            Err(BuluError::Other("Promise.all() requires an array of promises".to_string()))
        }
    }
    ...
    fn evaluate_promise_race(&mut self, call_expr: &crate::ast::CallExpr) -> Result<RuntimeValue> {
        if call_expr.args.len() != 1 {
            return Err(BuluError::Other("Promise.race() requires exactly one argument".to_string()));
        }
        ...
        if let crate::ast::Expression::Array(array) = array_expr {
            ...
            for element in &array.elements {
                ...
                match value {
                    RuntimeValue::Promise(id) => promise_ids.push(id as usize),
                    _ => return Err(BuluError::Other("Promise.race() requires an array of promises".to_string())),
                }
            }
            ...
            Err(BuluError::Other("Promise.race() requires an array of promises".to_string()))
        }
    }

    /// Evaluate a run expression (stub implementation for tests)
    pub fn evaluate_run_expression(&mut self, _run_expr: &crate::ast::RunExpr) -> Result<RuntimeValue> {
    ...
    pub fn promise_registry(&self) -> std::sync::Arc<std::sync::Mutex<crate::runtime::promises::PromiseRegistry>> {
        std::sync::Arc::clone(&self.promise_registry)
    }

    /// Evaluate await expression
    pub fn evaluate_await_expression(&mut self, await_expr: &crate::ast::AwaitExpr) -> Result<RuntimeValue> {
        // Check if we're in an async context
        if !self.is_in_async_context() {
            return Err(BuluError::Other("await can only be used inside async functions".to_string()));
        }
        ...
        match promise_value {
            RuntimeValue::Promise(promise_id) => {
                ...
                if let Some(promise) = registry.get_promise(promise_id as usize) {
                    ...
                    Err(BuluError::Other(format!("Promise with ID {} not found", promise_id)))
                }
            }
            _ => {
                Err(BuluError::Other("Cannot await non-Promise value".to_string()))
            }
        }
    }

    /// Evaluate channel expression
    fn evaluate_channel_expression(&mut self, channel_expr: &crate::ast::ChannelExpr) -> Result<RuntimeValue> {
        ...
        match channel_expr.direction {
            ChannelDirection::Send => {
                ...
                match channel_value {
                    ...
                    _ => Err(BuluError::Other("Cannot send to non-channel value".to_string()))
                }
            }
            ChannelDirection::Receive => {
                ...
                match channel_value {
                    ...
                    _ => Err(BuluError::Other("Cannot receive from non-channel value".to_string()))
                }
            }
        ...
        match expr {
            crate::ast::Expression::Identifier(ident) => {
                if let Some(value) = self.environment.get(&ident.name) {
                    ...
                    Err(BuluError::Other(format!("Undefined variable: {}", ident.name)))
                }
            }
    ...
    pub fn runtime_value_to_value(&self, runtime_value: RuntimeValue) -> Result<crate::ast::Expression> {
        // Stub implementation - convert back to expression
        match runtime_value {
            RuntimeValue::Int32(i) => Ok(crate::ast::Expression::Literal(crate::ast::LiteralExpr {
                value: crate::ast::LiteralValue::Integer(i as i64),
                position: crate::lexer::token::Position::new(1, 1, 0),
            })),
            RuntimeValue::String(s) => Ok(crate::ast::Expression::Literal(crate::ast::LiteralExpr {
    ...
    pub fn execute(&mut self) -> Result<RuntimeValue> {
        ...
        for ir_struct in &program.structs {
            ...
            self.struct_definitions.insert(ir_struct.name.clone(), struct_def);
        }
    ...
    pub fn execute_source(&mut self, source: &str) -> Result<RuntimeValue> {
        ...
        use crate::compiler::{SemanticAnalyzer, IrGenerator};
        ...
        match method_name {
            "toString" => {
                match object {
                    ...
                    _ => Ok(RuntimeValue::String("null".to_string())),
                }
            }
            _ => {
                // Check if it's a struct method
                if let RuntimeValue::Struct { name: struct_name, .. } = object {
                    if let Some(struct_def) = self.struct_definitions.get(struct_name).cloned() {
                        if let Some(function_name) = struct_def.methods.get(method_name) {
                            // Find the method function in the program
                            let program = self.program.clone().ok_or_else(|| 
                                BuluError::Other("No program loaded".to_string()))?;
                            
                            if let Some(method_function) = program.functions.iter()
                                .find(|f| f.name == *function_name) {
                                
                                // Prepare arguments: 'this' object + method arguments
    ...
    fn execute_instruction(&mut self, instruction: &IrInstruction) -> Result<()> {
        match instruction.opcode {
            IrOpcode::Call => {
                ...
                let result = match &instruction.operands[0] {
                    IrValue::Global(function_name) => {
                        ...
                        if let Some(builtin) = self.builtin_registry.get(function_name) {
                            ...
                            let user_function = if let Some(program) = &self.program {
                                program.functions.iter().find(|f| f.name == *function_name).cloned()
                            } else {
                                None
                            };
                    ...
                    IrValue::Register(_) => {
                        ...
                        match callee {
                            RuntimeValue::MethodRef { object, method_name } => {
                            ...
                            _ => {
                                return Err(BuluError::Other(format!(
                                    "Cannot call non-method value: {:?}", callee
                                )));
                            }
                        }
                    }
            ...
            IrOpcode::Eq => {
                ...
                let result = match (&left, &right) {
                    (RuntimeValue::Int32(i1), RuntimeValue::Int32(i2)) => RuntimeValue::Bool(i1 == i2),
                    (RuntimeValue::Int64(i1), RuntimeValue::Int64(i2)) => RuntimeValue::Bool(i1 == i2),
                    (RuntimeValue::Float32(f1), RuntimeValue::Float32(f2)) => RuntimeValue::Bool(f1 == f2),
                    (RuntimeValue::Float64(f1), RuntimeValue::Float64(f2)) => RuntimeValue::Bool(f1 == f2),
                    (RuntimeValue::String(s1), RuntimeValue::String(s2)) => RuntimeValue::Bool(s1 == s2),
            ...
            IrOpcode::Cast => {
                ...
                let target_type = match &instruction.operands[1] {
                    ...
                    _ => return Err(BuluError::Other("Cast target type must be a global".to_string())),
                };
            ...
            IrOpcode::StructConstruct => {
                if instruction.operands.is_empty() {
                    return Err(BuluError::Other(
                        "StructConstruct instruction requires at least one operand (struct name)".to_string(),
                    ));
                }
                ...
                for chunk in operands.chunks(2) {
                    let field_name = match &chunk[0] {
                        ...
                        _ => return Err(BuluError::Other("Field name must be a global".to_string())),
                    };
            ...
            IrOpcode::StructAccess => {
                ...
                let result = match &object {
                    RuntimeValue::Struct { name: struct_name, fields } => {
...
impl MockChannelRegistry {
    ...
    pub fn create_channel(&mut self, _type_id: crate::types::primitive::TypeId, _buffer_size: usize) -> u32 {
        let id = self.next_id;
        self.next_id += 1;
...
impl MockChannel {
    ...
    pub fn try_send(&mut self, value: RuntimeValue) -> Result<crate::runtime::channels::SendResult> {
